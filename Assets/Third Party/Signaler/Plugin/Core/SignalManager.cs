namespace echo17.Signaler.Core
{
    ...
    public class Signaler
    {
        ...
        /// <summary>
        /// Searches for a subscription with the proper subscriber, message action, and group
        /// </summary>
        /// <typeparam name="S">Signal Type</typeparam>
        /// <param name="group">Group</param>
        /// <returns>subscription if found</returns>
        private ISubscription _GetSubscription<S>(ISubscriber subscriber, MessageAction<S> action,
          long? group = null)
        {
          var signalKey = new SignalKey(typeof(S), typeof(NoResponse), group);
          IList subscriptionList;
        ...
        /// <param name="group">An optional group to narrow the broadcast to a small group. Default = null</param>
        /// <returns>True if successful</returns>
        public bool UnSubscribe<S>(ISubscriber subscriber, MessageAction<S> action,
          long? group = null)
        {
          var subscription = _GetSubscription<S>(subscriber, action, group);
